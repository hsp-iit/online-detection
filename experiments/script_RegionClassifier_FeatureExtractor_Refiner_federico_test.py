...
#import FALKONWrapper_KRR as falkon                                          # TODO remove this, use standard falkon
...
# ----------------------------------------------------------------------------------------
cfg_online_path =  'Configs/config_federico_server_icwt_21.yaml' #'Configs/config_federico_server_copy.yaml' #'Configs/config_federico_server_copy_rpn.yaml' #'' 'Configs/config_federico_server_icwt_21.yaml'
cfg_target_path =  'Configs/config_target_task_federico_server_icwt21.yaml' #'Configs/config_target_task_federico_server.yaml' #
...
if not icwt_21_objs:
    ...
    region_refiner = RegionRefiner(cfg_online_path)
    #regressors = torch.load('/home/IIT.LOCAL/fceola/workspace/ws_mask/python-online-detection/experiments/first_experiment/integration_tests_ep5_validation_set/regressors')
    #regressors = torch.load('/home/IIT.LOCAL/fceola/workspace/ws_mask/python-online-detection/experiments/first_experiment/integration_tests_ep5_validation_set_online_rpn/regressors')
    regressors = torch.load('/home/IIT.LOCAL/fceola/workspace/ws_mask/python-online-detection/experiments/first_experiment/integration_tests_ep5_validation_set/regressors_no_feat_norm')
    region_refiner.models = regressors
    """
    region_refiner = RegionRefiner(cfg_online_path)
    regressors = region_refiner.trainRegionRefiner()
    torch.save(regressors, os.path.join(regionClassifier.output_folder, 'regressors_no_feat_norm'))
    """
    #model = torch.load('/home/IIT.LOCAL/fceola/workspace/ws_mask/python-online-detection/experiments/first_experiment/integration_tests_ep5_validation_set/model_classifier_ep5_lambda0_0001_sigma5')
    model = torch.load('/home/IIT.LOCAL/fceola/workspace/ws_mask/python-online-detection/experiments/first_experiment/integration_tests_ep5_validation_set/model_classifier_ep5_lambda0_001_sigma20')
    #model = torch.load('/home/IIT.LOCAL/fceola/workspace/ws_mask/python-online-detection/experiments/first_experiment/integration_tests_ep5_validation_set_online_rpn/model_classifier_ep5_lambda0_001_sigma20')
    ...
    lambdas = [0.0001]#, 0.00001, 0.000001, 0.0000001, 0.001]
    sigmas = [25]#, 10, 15, 20, 25, 30, 50, 100, 1, 1000, 10000]
    for lam in lambdas:
        for sigma in sigmas:
            ...
            #torch.save(model, os.path.join(regionClassifier.output_folder, 'model_classifier_ep8_lambda%s_sigma%s_test_KRR' %(str(lam).replace(".","_"), str(sigma).replace(".","_")))) #TODO use previous
            ...
            if region_refiner is None:
                region_refiner = RegionRefiner(cfg_online_path)
                regressors = region_refiner.trainRegionRefiner()
            torch.save(regressors, os.path.join(regionClassifier.output_folder, 'regressors_test'))
            """
            region_refiner = RegionRefiner(cfg_online_path)
            regressors = torch.load('/home/IIT.LOCAL/fceola/workspace/ws_mask/python-online-detection/experiments/first_experiment/integration_tests_ep8_validation_set_icwt_21_online_rpn/regressors_test')
            region_refiner.models = regressors
            """
