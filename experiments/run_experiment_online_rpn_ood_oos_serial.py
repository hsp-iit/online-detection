...
parser.add_argument('--save_detector_segmentation_features', action='store_true', help='Save, in the features directory (in the output directory), features for detection and segmentation.')
...
feature_extractor = FeatureExtractor(cfg_target_task, config_file_rpn, train_in_cpu=args.CPU)

# Train RPN
if not args.no_rpn and not args.load_RPN_models:
...
elif not args.no_rpn and args.load_RPN_models:
...
elif args.no_rpn and args.load_RPN_models:
...
if args.load_detector_models:
    ...
    if not args.save_detector_segmentation_features and not args.load_detector_features:
        ...
        if args.save_detector_segmentation_features:
            feature_extractor.extractFeatures(is_train=True, output_dir=output_dir, save_features=args.save_detector_segmentation_features, extract_features_segmentation=True, use_only_gt_positives_detection=args.use_only_gt_positives_detection)
            del feature_extractor
            torch.cuda.empty_cache()
...
if not args.load_segmentation_models:
    ...
    if args.load_segmentation_features or args.save_detector_segmentation_features or args.load_detector_features or args.load_detector_models:
