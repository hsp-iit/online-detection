...
def extract_feature_proposals(cfg, dataset, model, transforms, icwt_21_objs=False, compute_average_recall_RPN = False, num_classes=30, is_train = True):
    ...
    for img_path in imset:
        ...
        with torch.no_grad():
            AR = model(image_list, None, gt_bbox=gt_bbox_boxlist, gt_label = gt_labels_torch, img_size = img_sizes, start_time = None,
                       compute_average_recall_RPN = compute_average_recall_RPN, num_classes=num_classes, gt_labels_list = gt_labels, is_train = is_train)
            if compute_average_recall_RPN:
    ...
    if compute_average_recall_RPN:
        return average_recall_RPN / count_procImg
    else:
        return None
